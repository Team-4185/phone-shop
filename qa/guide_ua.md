...
Коли ви запускаєте скрипт `setup.sh` або `setup.bat`, то він підіймає чотири контейнери:<br>
**Перший контейнер** - це база даних [Postgres](https://www.postgresql.org/), яка вже мінімально налаштована.<br>
**Другий контейнер** - це [Redis](https://redis.io/), який потрібен backend для перевірки відкликаних access та refresh токенів.<br>
**Третій контейнер** - це [MinIO](https://min.io/), де зберігаються зображення товарів.<br>
**Четвертий контейнер** - це сам застосунок, який розробляють `backend` розробники, і яким вам потрібно тестувати.

Ці контейнери вже пов'язані між собою, і вони вже правильно взаємодіють між собою.
...
6. Після цього вам потрібно перейти в директорію `phone-shop -> qa`, та переконатись, що ви бачите там файли, з
   назвами `setup`
7. Вам потрібно покласти файл зі змінними середовища з назвою `test.env` у директорію `phone-shop` або `phone-shop/qa`.
...
9. Запустіть скрипт:
    ...
    - якщо у вас Linux OS:
        ...
        - Дочекатись, доки в **Docker Desktop** у вкладці Containers не з'явиться контейнер з
          назвою `gadget-room-backend`
        - Працюйте..
10. Зробіть запит в браузері `http://localhost:8080/swagger-ui.html`

> **Примітка:** Якщо після оновлення гілки контейнер не піднявся через старі дані або Flyway checksum mismatch,
  не видаляйте контейнери руками в **Docker Desktop**. Запустіть `./setup.sh reset` на Linux/macOS або
  `setup.bat reset` на Windows. Ця команда видалить Docker volumes тільки для проєкту `gadget-room-backend`
  та підніме чисте середовище заново.
...
|:---------|--------------|
| Host     | `localhost`  |
| Port     | значення `DB_PORT` з `test.env` |
| Database | значення `DB_NAME` з `test.env` |
| Username | значення `DB_USERNAME` з `test.env` |
| Password | значення `DB_PASSWORD` з `test.env` |
...
| Параметер | Значення         |
|-----------|------------------|
| Username  | значення `MINIO_USERNAME` з `test.env` |
| Password  | значення `MINIO_PASSWORD` з `test.env` |
...
- `http://localhost:8080/swagger-ui.html` - отримати візуальний вигляд усього API
- `http://localhost:8080/v3/api-docs` - отримати JSON об'єкт усіх endpoint-ів
