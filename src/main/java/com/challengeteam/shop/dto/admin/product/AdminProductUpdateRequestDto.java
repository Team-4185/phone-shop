...
public record AdminProductUpdateRequestDto(
    ...
    @Size(max = 1000, message = "Description must be at most {max} characters long")
        String description,
    @DecimalMin(value = "0.00", message = "Price must be greater than {value}") BigDecimal price,
    ...
    @Size(min = 3, max = 64, message = "Sku must be between {min} and {max} characters")
        ...
        String sku,
    @Min(value = 0, message = "Stock must be greater than or equal to {value}") Integer stock,
